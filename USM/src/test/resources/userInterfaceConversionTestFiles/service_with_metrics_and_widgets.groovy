...
service {
	...
	//an example tomcat user interface.
	userInterface {
		...
		metricGroups = ([
			metricGroup {
				...
				metrics(["Total Process Cpu Time","Process Cpu Usage","Total Process Virtual Memory","Num Of Active Threads",["ddd", Unit.PERCENTAGE]])
			} ,
			metricGroup {
		...
		widgetGroups = ([
			...
			widgetGroup {
				name "TestingWidgetGroup"
				widgets([
					balanceGauge{metric = "overrriddenYAxisMetric"},
					barLineChart {
						metric "overrriddenYAxisMetric"
						axisYUnit Unit.REGULAR
					}
				])
			}
