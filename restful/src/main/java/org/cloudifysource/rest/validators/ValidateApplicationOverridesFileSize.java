...
/**
 * Validate the application overrides file size.
 * 
 * @author yael
 * 
 */
public class ValidateApplicationOverridesFileSize implements InstallApplicationValidator {
	...
	public void validate(final InstallApplicationValidationContext validationContext) throws RestErrorException {
		...
		if (appOverridesFile != null) {
			...
			if (length > getApplicationOverridesFileSizeLimit()) {
				throw new RestErrorException(CloudifyMessageKeys.APPLICATION_OVERRIDES_SIZE_LIMIT_EXCEEDED.getName(),
						appOverridesFile.getAbsolutePath(), length, getApplicationOverridesFileSizeLimit());
			}
		}
	}
