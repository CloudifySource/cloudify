...
if [ "$GSA_MODE" = "agent" ]; then
	...
	if [ "$NO_WEB_SERVICES" = "true" ]; then
		./cloudify.sh start-management -no-web-services -no-management-space -timeout 30 --verbose -auto-shutdown -cloud-file $CLOUD_FILE w|| error_exit $? "Failed starting management services"
	else
