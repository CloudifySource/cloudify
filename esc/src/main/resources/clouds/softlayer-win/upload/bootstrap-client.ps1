...
# Retry connecting to the target host at most MAX_RETRIES times
$MAX_RETRIES = 60
$Connected = $FALSE
$ErrorMessage = ""
$i =1
While ($i -le $MAX_RETRIES) {
	try {
		Connect-WSMan -Credential $cred $target -ErrorAction Stop
...
if (-Not $Connected) {
	Write-Host "WinRM connection failed with message '$ErrorMessage'"
	break
}
