...
class DebugHook{
	...
	DebugHook(serviceContext, mode="instead") {
		...
		this.groovyDebugCommandsFile = [
			this.serviceDir,
			"debug.groovy"
		].join(File.separator)

		this.debugMode = mode
		this.keepaliveFilename = [
			this.serviceDir,
			".cloudify_debugging.lock"
		].join(File.separator)
...
"""
	...
	def registerDebugSession() {
		File debugSessionsDir = new File([
			System.properties["user.home"],
			".gigaspaces",
			"debug_sessions"
		].join(File.separator))
		...
		writeTextFile(debugSessionFile, this.serviceDir)
	}
	...
	def prepareGroovyDebugCommands() {
		...
		writeTextFile(debugSessionFile, this.groovyDebugCommandsWrapper)
	}
	...
	def writeTextFile(File file, String fileContents) {
		if(!file.exists()) {
			...
			if(!directory.exists()) {
				...
				if(!result) {
					throw new IllegalArgumentException("Failed to create directory: " + directory.getAbsolutePath() + " for file: " + file.getAbsolutePath())
				}
	...
	def prepareDebugRc(debugTarget) {
		...
		writeTextFile(targetDebugrc, preparedTemplate.toString())
	}
}