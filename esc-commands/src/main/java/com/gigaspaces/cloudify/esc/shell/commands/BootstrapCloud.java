...
public class BootstrapCloud extends AbstractGSCommand {
	...
	protected Object doExecute() throws Exception {
		...
		File providerDirectory = new File(ShellUtils.getCliDirectory(),
				"plugins" + pathSeparator + "esc" + pathSeparator
						+ cloudProvider);
		...
		if (this.adminFacade != null) {
			...
			installer.setAdminFacade((AdminFacade) session
	...
	private File findCloudFile(File providerDirectory)
			throws FileNotFoundException {
		if (!providerDirectory.exists() || !providerDirectory.isDirectory()) {
			throw new FileNotFoundException(
					"Could not find cloud provider directory: "
							+ providerDirectory);
		}
